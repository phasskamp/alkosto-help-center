---
id: cómo-recuperar-tu-contraseña
title: "Cómo recuperar tu contraseña"
sidebar_label: "Cómo recuperar tu contraseña"
---

:::tip En este artículo aprenderás:
* Solicitar un enlace de recuperación de contraseña
* Acceder a tu correo para recuperar tu cuenta
* Establecer una nueva contraseña segura Selecciona tu dispositivo: [Computadora](#desktop-section) [Móvil](#mobile-section)
:::


## Instrucciones para Computadora #

### 1. Accede a "Mi cuenta"

En la esquina superior derecha de la página principal, haz clic en el botón **"Mi cuenta"**.


![](https://41k0st0.my.site.com/ayuda/servlet/rtaImage?eid=ka0Qm0000001g0P&feoid=00N8c00000H564C&refid=0EMQm00000H6hYL)

#

### 2. Ingresa tu correo electrónico

En el campo de texto, escribe el correo electrónico asociado a tu cuenta Alkosto y presiona **"Continuar"**. #


### 3. Selecciona "Mail y contraseña"

En las opciones de inicio de sesión, haz clic en **"Mail y contraseña"**.


![](https://41k0st0.my.site.com/ayuda/servlet/rtaImage?eid=ka0Qm0000001g0P&feoid=00N8c00000H564C&refid=0EMQm00000H6fZo)

#

### 4. Solicita recuperación de contraseña

Haz clic en el enlace **"Olvidé mi contraseña"** y luego en el botón **"Continuar"**.


![](https://41k0st0.my.site.com/ayuda/servlet/rtaImage?eid=ka0Qm0000001g0P&feoid=00N8c00000H564C&refid=0EMQm00000H6kUc)

**💡 Consejo:** Verifica que tu correo esté escrito correctamente antes de continuar. #

### 5.

Revisa tu correo electrónico Abre tu bandeja de entrada.

Te llegará un correo de Alkosto con un enlace para recuperar tu contraseña. **⚠️ Importante:** Si no ves el correo, revisa tu carpeta de spam o correo no deseado. #


### 6.

Crea tu nueva contraseña Haz clic en el enlace del correo.

Se abrirá una nueva ventana donde podrás **registrar tu nueva contraseña**.


## Instrucciones para Móvil #

### 1. Abre el menú de 'Mi cuenta'

Toca el ícono de 'Mi cuenta' en la esquina inferior izquierda de la app o sitio móvil.


![](https://41k0st0.my.site.com/ayuda/servlet/rtaImage?eid=ka0Qm0000001g0P&feoid=00N8c00000H564C&refid=0EMQm00000H6ndl)

#

### 2. Ingresa tu correo

Escribe tu correo electrónico en el campo de texto y toca **"Continuar"**. **💡 Tip móvil:** Usa el teclado de tu dispositivo para autocompletar tu correo si lo tienes guardado. #


### 3. Elige "Mail y contraseña"

Desliza hacia abajo si es necesario y toca la opción **"Mail y contraseña"**.


![](https://41k0st0.my.site.com/ayuda/servlet/rtaImage?eid=ka0Qm0000001g0P&feoid=00N8c00000H564C&refid=0EMQm00000H6o8P)

#

### 4.

Toca "Olvidé mi contraseña" Busca el enlace **"Olvidé mi contraseña"** debajo del campo de contraseña y tócalo.

Luego toca **"Continuar"**.


![](https://41k0st0.my.site.com/ayuda/servlet/rtaImage?eid=ka0Qm0000001g0P&feoid=00N8c00000H564C&refid=0EMQm00000H6h3k)

#

### 6.

Abre tu app de correo Sal de la app/sitio de Alkosto y abre tu aplicación de correo electrónico (Gmail, Outlook, etc.). **📱 Nota:** El correo puede tardar unos minutos en llegar.

Actualiza tu bandeja de entrada deslizando hacia abajo. #


### 7.

Crea tu nueva contraseña Toca el enlace en el correo.

Se abrirá tu navegador donde podrás crear tu **nueva contraseña**. [↑ Volver arriba](#desktop-section) **🔐 Recomendaciones para una contraseña segura:**

* Mínimo 8 caracteres
* Incluye mayúsculas y minúsculas
* Usa números y símbolos especiales
* No uses información personal obvia

## ✓ ¡Listo! Contraseña actualizada ✅ Tu contraseña ha sido actualizada exitosamente.

Ya puedes iniciar sesión con tu nueva contraseña.

### ¿Problemas frecuentes?
* **No recibí el correo:** Espera unos minutos y revisa tu carpeta de spam
* **El enlace expiró:** Solicita un nuevo enlace siguiendo los mismos pasos
* **No recuerdo mi correo:** Contacta a nuestro servicio al cliente 

:::info ¿Necesitas más ayuda?
Si tienes dudas, puedes [contactar con soporte](/docs/contacto).
:::
